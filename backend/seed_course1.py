"""
Seed script: Year 1 - Foundation of Communication & Self-Management

Usage:
1. Put the raw MCQ text into: content/year1_mcqs.txt
2. Run from the backend directory:
   python seed_course1.py
"""
import re
import sys
import uuid
...
COURSE = {
    "title": "Foundation of Communication & Self-Management",
    "description": "Build confidence, clarity, and professional mindset. Develop basic communication, interpersonal skills and prepare for collaborative academic life.",
    "year": 1,
    "order": 1,
}

RAW_CONTENT_PATH = Path(__file__).resolve().parent / "content" / "year1_mcqs.txt"
...
def run_migrations():
    ...
    with engine.connect() as conn:
        if is_pg:
            # PostgreSQL: add columns if missing
            conn.execute(text("""
                DO $$ BEGIN
                    IF NOT EXISTS (
                        ...
                        ALTER TABLE quizzes ADD COLUMN lesson_id CHAR(36) REFERENCES lessons(lesson_id) ON DELETE CASCADE;
                    END IF;
                END $$;
            """))
            conn.execute(text("""
                DO $$ BEGIN
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns 
                        WHERE table_name='options' AND column_name='explanation'
                    ) THEN
                        ALTER TABLE options ADD COLUMN explanation TEXT;
                    END IF;
                END $$;
            """))
            conn.execute(text("""
            ...
            conn.execute(text("""
                DO $$ BEGIN
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns 
                        WHERE table_name='questions' AND column_name='placement_relevance'
                    ) THEN
                        ALTER TABLE questions ADD COLUMN placement_relevance VARCHAR(255) DEFAULT 'Core placement readiness';
                    END IF;
                END $$;
            """))
        else:
            # SQLite: try ALTER TABLE, ignore if column already exists
            for stmt in [
                "ALTER TABLE quizzes ADD COLUMN lesson_id CHAR(36)",
                "ALTER TABLE options ADD COLUMN explanation TEXT",
                "ALTER TABLE questions ADD COLUMN difficulty_level VARCHAR(50) DEFAULT 'Medium'",
                "ALTER TABLE questions ADD COLUMN placement_relevance VARCHAR(255) DEFAULT 'Core placement readiness'",
            ]:
...
def seed(db: Session | None = None, *, raw_text: str | None = None, apply_migrations: bool = True) -> bool:
    if apply_migrations:
        run_migrations()
    ...
    if not mcqs:
        raise ValueError("No MCQs were parsed from the provided Year 1 content.")
    ...
    try:
        ...
        db.commit()
        print(f"[DONE] Seeded Year 1 module with {len(mcqs)} MCQs.")
        return True
    except Exception:
        db.rollback()
        raise
