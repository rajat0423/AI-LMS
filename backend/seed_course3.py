"""
Seed script: Year 3 - Employability & Interview Readiness

Usage:
1. Put the raw MCQ text into: content/year3_mcqs.txt
2. Run from the backend directory:
   python seed_course3.py
"""
import re
import sys
import uuid
...
COURSE = {
    "title": "Employability & Interview Readiness",
    "description": "Predefined learning pathways, advanced resume creation, professional branding on LinkedIn, behavioral and situational interview handling, and career readiness for third-year learners.",
    "year": 3,
    "order": 3,
}

RAW_CONTENT_PATH = Path(__file__).resolve().parent / "content" / "year3_mcqs.txt"
...
def seed(db: Session | None = None, *, raw_text: str | None = None) -> bool:
    ...
    if not mcqs:
        raise ValueError("No MCQs were parsed from the provided Year 3 content.")
    ...
    if existing:
        print(f"Module '{COURSE['title']}' for Year 3 already exists (id={existing.module_id}). Skipping.")
    ...
    try:
        ...
        db.commit()
        print(f"[DONE] Seeded Year 3 module with {len(mcqs)} MCQs.")
        return True
    except Exception:
        db.rollback()
        raise
