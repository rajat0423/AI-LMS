"""
Seed script: Year 2 - Professional Communication & Interpersonal Effectiveness

Usage:
1. Put the raw MCQ text into: content/year2_mcqs.txt
2. Run from the backend directory:
   python seed_course2.py
"""
import re
import sys
import uuid
...
COURSE = {
    "title": "Professional Communication & Interpersonal Effectiveness",
...
RAW_CONTENT_PATH = Path(__file__).resolve().parent / "content" / "year2_mcqs.txt"
...
def seed(db: Session | None = None, *, raw_text: str | None = None) -> bool:
    ...
    if not mcqs:
        raise ValueError("No MCQs were parsed from the provided Year 2 content.")
    ...
    if existing:
        print(f"Module '{COURSE['title']}' for Year 2 already exists (id={existing.module_id}). Skipping.")
    ...
    try:
        ...
        db.commit()
        print(f"[DONE] Seeded Year 2 module with {len(mcqs)} MCQs.")
        return True
    except Exception:
        db.rollback()
        raise
