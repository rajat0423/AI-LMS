...
function Navbar() {
    ...
    return (
        <>
            <header className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm flex-none">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    
                    {/* Left: Brand */}
                    <div className="flex items-center gap-3">
                        <button 
                            className="lg:hidden p-2 -ml-2 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors rounded-lg"
                        ...
                        <NavLink to="/dashboard" className="flex items-center group">
                            ...
                            <img src="/logo.webp" alt="Aao Seekhe Live" className="h-10 w-auto group-hover:scale-105 transition-transform" />
                        </NavLink>
                    </div>

                    {/* Center: Desktop Navigation — FULL dark mode support */}
                    <div className="hidden lg:flex items-center gap-1.5 p-1 bg-slate-100/80 dark:bg-slate-800/80 rounded-full border border-slate-200 dark:border-slate-700">
                        {navLinks.map((link) => {
                            ...
                            return (
                                <NavLink
                                    key={link.name}
                                    to={link.path}
                                    className={`relative px-5 py-2 text-sm font-semibold rounded-full transition-all duration-200 flex items-center gap-2 z-10 tracking-wide ${
                                        isActive 
                                ...
                                >
                                    {isActive && (
                                        <motion.div
                                            ...
                                            className="absolute inset-0 bg-white dark:bg-slate-700 rounded-full shadow-sm border border-slate-200/60 dark:border-slate-600/60 -z-10"
                                            transition={{ type: "spring", stiffness: 300, damping: 30 }}
                                        />
                                    )}
                                    <link.icon size={16} className={isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'} />
                                    {link.name}
                                </NavLink>
                            );
                        })}
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 lg:gap-5">
                        ...
                        <button 
                            onClick={toggleTheme} 
                            className="text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-amber-400 transition-colors p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800"
                        ...
                        <button 
                            ...
                            className="hidden md:flex items-center gap-2 pl-3 pr-2 py-1.5 bg-slate-100 hover:bg-slate-200/80 border border-slate-200 rounded-lg text-slate-400 hover:text-slate-600 transition-colors group dark:bg-slate-800 dark:border-slate-700 dark:text-slate-500 dark:hover:text-slate-300 dark:hover:bg-slate-700"
                        >
                            <Search size={16} />
                            <span className="text-sm font-medium pr-8">Search...</span>
                            <kbd className="hidden lg:inline-block px-2 py-0.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-xs font-semibold text-slate-400 dark:text-slate-500 tracking-wide group-hover:border-slate-300 dark:group-hover:border-slate-600">
                                ⌘K
                            </kbd>
                        </button>
                        ...
                        <div className="relative">
                            <button 
                                ...
                                className="relative text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800"
                                aria-label="Notifications"
                            >
                                <Bell size={20} />
                                <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white dark:border-slate-900 drop-shadow-sm"></span>
                            </button>
                        ...
                        <div className="relative">
                            <button 
                                ...
                                className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-bold shadow-sm border border-indigo-200/50 dark:border-indigo-700/50 hover:shadow-md transition-all active:scale-95"
                            >
